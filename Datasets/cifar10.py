...
class CIFAR10(Dataset):
    ...
    def init(root, num_query, num_train):
        ...
        query_per_class = num_query // 10   #100
        train_per_class = num_train // 10   #500
        ...
        # list_train_index = [i for i in train_index]
        retrieval_index = np.array(list(set(range(data.shape[0])) - set(list_query_index)), dtype=int)

        # Split data, targets
        CIFAR10.QUERY_IMG = data[query_index, :]  # 1000张
        CIFAR10.QUERY_TARGET = targets[query_index]

        CIFAR10.TRAIN_IMG = data[train_index, :]  # 5000张
        CIFAR10.TRAIN_TARGET = targets[train_index]

        CIFAR10.RETRIEVAL_IMG = data[retrieval_index, :]  # 59000张(不包含查询的1000张)
